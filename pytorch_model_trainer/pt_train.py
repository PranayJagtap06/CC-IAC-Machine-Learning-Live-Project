...
from data_loader import CreateDataset_, load_data_objs
...
def model_eval(
    ...
    acc_metric_func = MulticlassAccuracy(
        num_classes=classes_num, average="micro", sync_on_compute=False
    ).to(0)
    f1_metric_func = MulticlassF1Score(
        num_classes=classes_num, average="macro", sync_on_compute=False
    ).to(0)
    ...
    mcc_metric_func = MulticlassMatthewsCorrCoef(
        num_classes=classes_num, sync_on_compute=False
    ).to(0)
    pred_labels = np.array([{"targets": [], "preds": []}])
    total_loss = 0
    total_samples_ = 0
    with torch.inference_mode():
        for source, targets in track(
            val_dtl,
    ...
    accuracy = acc_metric_func.compute() * 100
    f1_score = f1_metric_func.compute() * 100
    precision = precision_metric_func.compute() * 100
    recall = recall_metric_func.compute() * 100
    mcc = mcc_metric_func.compute() * 100
    bal_acc = (
    ...
    print(
        f"\nLoss --> {avg_loss:.4f}% | Accuracy --> {accuracy:.4f}% | Balanced Acc --> {bal_acc:.4f}% | F1 Score --> {f1_score:.4f}% | Precision --> {precision:.4f}% | Recall --> {recall:.4f}% | Matthews Correlation Coefficient --> {mcc:.4f}%"
    )
    print(
...
def main(
    ...
    train_dtl, val_dtl, model, criterion, optimizer = load_data_objs(
        batch_size,
        rank,
        world_size,
        xtrain_path,  # type: ignore
        ytrain_path,
        xval_path,
        yval_path,
        use_gpu,
    ...
    trainer = CustomTrainer(
        model=model,
        train_data=train_dtl,
        val_data=val_dtl,
        criterion=criterion,
        optimizer=optimizer,
        gpu_id=rank,  # type: ignore
        save_path=save_path,
        use_gpu=use_gpu,
        patience=patience,
...
if __name__ == "__main__":
    ...
    # parser.add_argument(
    #     "--in_features", default=2, type=int, help="Number of classes (default: 2)"
    # )
    parser.add_argument(
        "--model_save_path",
        default="./checkpoints",
    ...
    parser.add_argument(
        "--no-gpu", dest="gpu", action="store_false", help="Train on CPU"
    )
